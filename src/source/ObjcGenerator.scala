...
class ObjcGenerator(spec: Spec) extends BaseObjcGenerator(spec) {
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface) {
    ...
    def writeSwiftFuncDecl(method: Interface.Method, w: IndentWriter) {
      val ret = marshal.returnType(method.ret)
      val decl = s"func ${idObjc.method(method.ident)}"
      val a = s"$decl ("
      val e = s") -> async $ret { cruxAPI->$decl() }"
      writeAlignedSwiftCall(w, a, method.params, e, p => (idObjc.field(p.ident), s"(${marshal.paramType(p.ty)})${idObjc.local(p.ident)}"))
    }
    ...
    writeSwiftFile(marshal.headerName(ident) + ".swift", origin, w => {
      val sfx = s"Async"
      w.w(s"class $self$sfx {")
      w.wl
      writeDoc(w, doc)
      
      w.w(s"private let cruxAPI: $self?") 
      w.wl
      w.w(s"init() {")
      w.w(s"self.cruxAPI = $self->create()")
      w.w(s"}")
      for (m <- i.methods) {
        if (!m.static || (!spec.objcGenProtocol && m.lang.objc)) {
          w.wl
          writeMethodDoc(w, m, idObjc.local)
          writeSwiftFuncDecl(m, w)
          w.wl(";")
        }
      }
      for (c <- i.consts if !marshal.canBeConstVariable(c)) {
  ...
  def writeSwiftFile(fileName: String, origin: String, f: IndentWriter => Unit) {
    createFile(spec.objcOutFolder.get, fileName, (w: IndentWriter) => {
      w.wl("// AUTOGENERATED FILE - DO NOT MODIFY!")
      w.wl("// This file was generated by Djinni from " + origin)
      w.wl
